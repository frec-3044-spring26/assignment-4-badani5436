...
- You can see ice-off day varies a lot year to year, but there's a slight downward trend over time.
...
- The slope should be negative. As years increase (more recent times), we expect ice to melt earlier in the year due to climate warming, meaning the ice-off day of year (DOY) should decrease over time. A negative slope would reflect this earlier ice-off trend.
...
Then, use the function `summary` to explore the r9esult of the `lm` function.
...
-  The slope is negative (-0.106 days/year). This does match the expectation from Question 2 — as years increase, the ice-off day of year decreases, meaning that the ice is melting earlier over time at Lake Sunapee.
...
**Answer 4:**
- The R2 for the regression is 0.2055. This means that year explains only about 20.5% of the variation in ice-off day of year. This is a relatively low R2, which means that year and ice-off DOY are not strongly related.
...
- The distribution meets the assumptions of linear regression — residuals are centered near zero (indicating no bias) though the distribution is slightly right-skewed with a longer tail on the positive side. Overall, the assumption of normally distributed residuals is reasonably satisfied.
...
**Answer 7:**
- The p-value for the slope is 2.5e-09. This is an extremely small p-value. This means there is a very low probability of observing a slope this extreme if the true slope were actually zero, giving us evidence that the slope is significantly different from zero.
...
**Answer 8:**
- Based on the slope of -0.106 days/year, the ice-off date is shifting earlier by about 0.106 days per year. To achieve a 7-day change, it would take approximately 66 years (7 ÷ 0.106). This is meaningful, as it indicates that lake ice is consistently melting earlier due to long-term climate warming.
...
**Answer 9:**
- The model is pretty good. The slope is statistically significant (p = 2.5e-09) and residuals are approximately normally distributed, satisfying key assumptions. However, the low R² (0.21) means year only explains about 21% of the variation in ice-off DOY, giving the model weak predictive power for any specific year.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(
    title = "Lake Sunapee Ice-Off DOY with Linear Regression",
    x = "Year",
    y = "Ice-Off Day of Year (DOY)"
  )
```
...
#confidence interval
lower_ci <- pred_2040 + quantile(sunapee$resid, 0.05)
upper_ci <- pred_2040 + quantile(sunapee$resid, 0.95)
...
upper_ci - lower_ci
```
-The predicted ice-off DOY for 2040 is day 102.7 
- The 90% confidence interval is 83.2 to 116.8 (lower CI = 83.23, upper CI = 116.81).
- The confidence interval is 33.6 days wide, meaning our prediction has considerable uncertainty — the actual ice-off date could range by over a month from the prediction.
...
sunapee_early <- sunapee %>% filter(year < 1970)
sunapee_late  <- sunapee %>% filter(year >= 1970)

#separate models
fit_early <- lm(ice_off_doy ~ year, data = sunapee_early)
fit_late  <- lm(ice_off_doy ~ year, data = sunapee_late)
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point(aes(color = year < 1970)) +
  geom_abline(intercept = fit_early$coefficients[1],
              slope = fit_early$coefficients[2],
              color = "steelblue", linewidth = 1) +
  geom_abline(intercept = fit_late$coefficients[1],
              slope = fit_late$coefficients[2],
              color = "tomato", linewidth = 1) +
  scale_color_manual(values = c("tomato", "steelblue"),
                     labels = c("1970 and after", "Before 1970"),
                     name = "Period") +
  labs(
    title = "Lake Sunapee Ice-Off DOY: Early vs. Late Period",
    x = "Year",
    y = "Ice-Off Day of Year (DOY)"
  )
```
...
-  Yes, the ice-off day of year is changing more quickly in more recent years at Lake Sunapee. The slope for the early period (before 1970) is -0.110 days/year, while the slope for the late period (1970 and after) is -0.302 days/year — nearly three times steeper. Both slopes are statistically significant (p = 0.00108 and p = 0.000161). This is also visible in the plot, where the red regression line (1970 and after) is noticeably steeper than the blue line (before 1970). This acceleration suggests that climate warming has been having an increasingly stronger effect on lake ice in more recent decades.
